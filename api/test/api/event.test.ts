...
import { eventUrl, newValidOrganizer, newValidUser, sighupUrl } from './common';
    ...
    describe("Creating Event", () => {
        ...
        describe("WHEN not Login in as a Organizer", () => {
            ...
            describe("WHEN Login in as a User", () => {
                ...
                it("SHOULD return a 401 status code AND Error obj", async () => {
                    request(app).post(eventUrl()).set('authorization', `Bearer ${accessToken}`).send({}).expect(401)
                })
            })
        ...
        //     //             const response = await request(app).post(privateEventUrl).set("Authorization", `Bearer ${organizerToken}`).send(makeEvent({ category: [category.id] }));
        ...
        //     //             await request(app).delete(privateEventUrl + organizer.id).set("Authorization", `Bearer ${organizerToken}`);
        //     //         });
        ...
        //     //             const response = await request(app).post(privateEventUrl).set("Authorization", `Bearer ${organizerToken}`).send(makeEvent({ category: ["abc123"] }));
        ...
        //     //             const response = await request(app).post(privateEventUrl).set("Authorization", `Bearer ${organizerToken}`).send(makeEvent({}));
        //     //             expect(response.status).toBe(200);
        //     //             expect(response.body.body).toMatchObject({
        //     //                 ...makeEvent({}),
        ...
        //     //             const response = await request(app).post(privateEventUrl).set("Authorization", `Bearer ${organizerToken}`).send({});
